...
const LS_KEY = 'bookly_demo_state_v2';

const enrichTenants = (tenants) => tenants.map((t) => ({ ...t, businessHours: t.businessHours || DEFAULT_BUSINESS_HOURS }));
const enrichEmployees = (employees) => employees.map((e) => ({ ...e, schedule: e.schedule || DEFAULT_EMPLOYEE_SCHEDULE }));

const initialState = () => ({
  tenants: enrichTenants(TENANTS),
  employees: enrichEmployees(EMPLOYEES),
  services: SERVICES,
  clients: CLIENTS,
...
const loadState = () => {
  ...
  try {
    ...
    merged.tenants = enrichTenants(merged.tenants);
    merged.employees = enrichEmployees(merged.employees);
    return merged;
  } catch {
    return initialState();
  }
