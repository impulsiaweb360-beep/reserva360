...
// Si el empleado NO tiene horario propio configurado, se asume el horario del negocio.
export function getWorkingWindows(date, tenant, employee) {
  ...
  // Si el empleado no tiene schedule, hereda el horario del negocio
  const empSchedule = validOrDefault(employee?.schedule, bizHours);
...
export function isWithinBusinessHours({ start, end, tenant, employee }) {
  ...
  const dow = day.day();
  const bizHours = validOrDefault(tenant?.businessHours, DEFAULT_BUSINESS_HOURS);
  // Si el empleado no tiene schedule propio, hereda el horario del negocio
  const empSchedule = validOrDefault(employee?.schedule, bizHours);
  const biz = bizHours[dow] || bizHours[String(dow)];
  const emp = empSchedule[dow] || empSchedule[String(dow)];
